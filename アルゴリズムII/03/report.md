...
### コード
- [two_three_tree.cpp](two_three_tree/two_three_tree.cpp)

### 実行結果
[test.sh](two_three_tree/test.sh)でコンパイルと実行を行いました。

[実行結果](two_three_tree/result.txt)

### 解説
- この実装では、値の集合を直接木に細かく反映するのではなく、いったんソート済みの `values` を基準に木を作り直す方針を取っている
- `rebuild()` が全体の中心で、挿入と削除のたびに 2-3 木の条件を満たす形へ再構成している
- `build_subtree()` は、区間を再帰的に分割して高さのそろった部分木を作る関数であり、この関数が木の形そのものを決めている
- `search_node()` は、各ノードのキー比較だけで次に進む子を決めることで、2-3 木の探索経路が一意になる性質を使っている
- `render_box()` と `print_rendered()` は処理本体ではなく表示専用で、木の構造を追いやすくするための補助機能になっている
- クラスを使うことで、木の管理・検索・描画・破棄をひとまとめにし、メモリ管理も含めて扱いやすくしている
...
### コード
- [patricia_tree.cpp](patricia_tree/patricia_tree.cpp)

### 実行結果
[test.sh](patricia_tree/test.sh)でコンパイルと実行を行いました。

[実行結果](patricia_tree/result.txt)

### 解説
- パトリシア木は、共通接頭辞を 1 ノードに圧縮して持つ trie であり、通常の trie より経路が短い
- `insert(word)` の本質は単純な追加ではなく、既存ラベルとの一致部分を見て必要ならノードを分割し、圧縮された経路を保つことにある
- `search(word)` は、文字を 1 文字ずつたどるのではなく、ラベル単位でまとめて照合するので、途中で不一致が分かれば早く失敗できる
- `erase(word)` は削除後の後始末が重要で、終端フラグを落としたあとに不要になったノードを消し、1 本しか子を持たないノードは結合して圧縮を回復している
- `renderTree(node, is_root)` は表示専用の関数で、圧縮されたラベルを崩さずに木の分岐関係を見える形へ変換している
- 補助関数の `trimRight`、`putText`、`putToken`、`cellWidth` は、描画の整形を分担していて、本体アルゴリズムとは切り離されている
